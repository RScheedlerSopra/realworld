...
4. Read the template. If parts are not clear in the feature specification from the user and for other questions to make the feature clear, use the #askQuestions tool (max 2 times) for every unclear part.
...
6. Use #runSubagent tool to spawn a subagent (default agent type). Ask the subagent to check the generated spec.md against the template. It should detect things that are missing and things that are undesired like technical details.